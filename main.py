...
아래 내용은 사용자가 직접 작성한 커스텀 요청사항입니다. 아래 내용을 무조건 최우선으로 반영하여 문제를 출제하세요.
"{custom_prompt}"
...
3. '단답식' 유형별 규칙:
   - 단답식: 
     ...
     * 참/거짓(O/X)을 묻는 문제는 출제하지 마세요. 
     * 정답(answer)은 무조건 15글자 이내의 핵심 명사, 고유명사, 혹은 숫자, 정확한 전문용어로만 작성하세요.
     * 정답에 특수기호, 쉼표(,), '및/과/와' 같은 연결어나 조사가 절대 포함되지 않게 하세요. (문장형, 서술형 절대 금지)
